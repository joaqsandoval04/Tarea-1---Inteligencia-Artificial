...
def recalcular_camino(laberinto, solver, agente_pos, salidas_visitadas):
    if agente_pos is None:
        agente_pos = laberinto.find_agent_position()
    ...
    pos_inicial = list(zip(*np.where(laberinto_copy == 2)))[0]
    laberinto_copy[pos_inicial] = 0
    laberinto_copy[agente_pos] = 2
    ...
    for salida in salidas_visitadas:
        if laberinto_copy[salida] == 3:
            laberinto_copy[salida] = 1

    # Ejecutar A* desde la posición actual
    caminos, exito = solver.solve(laberinto_copy)

    # Restaurar valores las salidas
    for salida in salidas_visitadas:
        laberinto_copy[salida] = 3
